...
draw_raster <- function(r,
  ...
  if (!is.null(bb)){
    ...
    cols <- 
    plot(st_geometry(pp), 
         col = sf::sf.colors(n = nrow(pp), alpha = 0.5, categorical	= TRUE), 
         alpha = 0.7,
         border = "black", 
         lwd = 3,
         add = TRUE)
...
map(database = "world", col = "grey50")
box(col = 'black')
plot(newR, add = TRUE, legend = FALSE)
map(database = "world", col = "grey50", add = TRUE)
```
