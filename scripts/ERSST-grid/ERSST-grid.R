...
ERSST_nc_nav <- function(x,
  ...
  bb2 <- bb + c(-res[1]/2, res[1]/2, -res[2]/2, res[2]/2)
  ...
  list(bb = bb,
       ...
       ext = to180BB(c(lon[iW] - res[1]/2, lon[iE] + res[1]/2,
               lat[iS] - res[2]/2, lat[iN] + res[2]/2)),
...
ERSST_get_grid <- function(type = c("month", "ltm")[1],
  ...
  if (FALSE){
    type = c("month", "ltm")[1]
    bb = c(-71, -64, 40, 46)
    daterange = c('2018-01-01', '2018-04-04')
    outpath = NA
  }
  ...
  nav <- ERSST_nc_nav(x,
                      bb = to360BB(bb),
                      dates = dates)
  SS <- ERSST_fetch_month(x, nav)
  SS <- raster::setZ(x, dates)
